package com.sou.dw.jpa.dao;

import com.google.inject.Provider;
import com.google.inject.persist.Transactional;
import com.sou.dw.jpa.entity.Player;
...
public class DaoImpl implements Dao {
    ...
    private static final String QUERY_SELECT_ALL = "SELECT o FROM %s o ORDER BY o.%s";
    ...
    @Override
    public <T> List<T> findAll(final Class clazz, final String orderBy) {
        final String query = String.format(QUERY_SELECT_ALL, clazz.getSimpleName(), orderBy);
	...
	@Override
	public <T, ID> void removeAll(Class<T> clazz) {
		// entityManager.get().createQuery(String.format("DELETE FROM %s", clazz.getSimpleName()) ).executeUpdate();
		String selectQuery = String.format("DELETE FROM %s", clazz.getSimpleName()) ;  
		List<Player> playerToRemove = findAll(clazz, "id")  ;
